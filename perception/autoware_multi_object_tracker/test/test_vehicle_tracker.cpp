...
// ---------------------------------------------------------------------------------------------
// getAxleLateralVariance + correctWheelAnchor observability floor — a one-sided cluster must not
// drive the lateral/yaw estimate to be more certain than the far (unobserved) axle already is.
// ---------------------------------------------------------------------------------------------

// After straight-line evolution the differential process noise has hit the front axle only, so the
// front axle point carries more lateral variance than the rear — the growing prior the floor keys
// on. (Heading is zero, so the lateral axis is +y and the projection reduces to P(Y*,Y*).)
TEST(GetAxleLateralVariance, FrontExceedsRearAfterEvolution)
{
  BicycleMotionModel model = makeEvolvedVehicle();
  const double front = model.getAxleLateralVariance(/*front_point=*/true);
  const double rear = model.getAxleLateralVariance(/*front_point=*/false);
  EXPECT_GT(front, 0.0);
  EXPECT_GT(rear, 0.0);
...
namespace
{
// A minimal predicted tracker box at the origin with heading 0 (lateral axis = +y).
types::DynamicObject makePrediction(const double tracker_width)
{
  types::DynamicObject pred;
  pred.pose.position.x = 0.0;
  pred.pose.position.y = 0.0;
  pred.pose.position.z = 0.0;
  pred.pose.orientation.w = 1.0;  // identity -> yaw 0
  pred.shape.dimensions.x = 4.5;
  pred.shape.dimensions.y = tracker_width;
  pred.shape.dimensions.z = 1.5;
  return pred;
}
}  // namespace

// One-sided cluster: the polygon is far narrower than the tracker, so one_sidedness * far-axle
// prior dominates the fixed dead-zone variance and is folded into the lateral (y at yaw 0)
// measurement block — the measurement can no longer out-certain the prior, so it cannot lever the
// box.
TEST(CorrectWheelAnchorObservabilityFloor, OneSidedClusterFloorsLateralVariance)
{
  const double tracker_width = 2.0;  // tracker_half = 1.0
  const double polygon_width = 0.4;  // polygon_half = 0.2 -> one_sidedness = 0.8
  const double far_axle_lateral_var = 5.0;

  types::DynamicObject pred = makePrediction(tracker_width);
  geometry_msgs::msg::Point anchor;
  anchor.x = -2.0;  // longitudinal offset drops out at yaw 0
  anchor.y = -0.7;  // near-side lateral offset

  std::array<double, 36> pose_cov{};
  pose_cov[XYZRPY_COV_IDX::Y_Y] = 0.1;
  correctWheelAnchor(pred, polygon_width, anchor, pose_cov, far_axle_lateral_var);

  const double base_var = (1.0 - 0.2) * (1.0 - 0.2);         // dead-zone half-width^2 = 0.64
  const double expected_floor = 0.8 * far_axle_lateral_var;  // one_sidedness * prior = 4.0
  ASSERT_GT(expected_floor, base_var);                       // the floor is what binds here
  EXPECT_NEAR(pose_cov[XYZRPY_COV_IDX::Y_Y], 0.1 + expected_floor, 1e-9);
  EXPECT_NEAR(
    pose_cov[XYZRPY_COV_IDX::X_X], 0.0, 1e-9);  // nothing leaks into the longitudinal axis
}

// Full-width observation: the far side is in view, so one_sidedness -> 0 (and the dead-zone -> 0).
// The floor vanishes and the lateral variance is left untouched, so yaw can still update.
TEST(CorrectWheelAnchorObservabilityFloor, FullWidthObservationNoFloor)
{
  const double tracker_width = 2.0;
  const double polygon_width = 2.0;  // equals tracker
  const double far_axle_lateral_var = 5.0;
  ...
  EXPECT_NEAR(pose_cov[XYZRPY_COV_IDX::Y_Y], 0.1, 1e-9);
}
